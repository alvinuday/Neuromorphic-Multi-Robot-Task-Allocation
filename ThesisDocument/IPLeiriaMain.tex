% Author Name: José Areia 
% Author Contact: jose.apareia@gmail.com
% Version: 2.2.13 - 2026/01/06
% Public Repository: https://github.com/joseareia/ipleiria-thesis
% Wiki/Getting Help: https://github.com/joseareia/ipleiria-thesis/wiki

%%% Document Options %%%
\documentclass[
    language=english,
    school=estg,
    docstage=final,
    media=paper,
    bookprint=false,
    linkcolor=red!45!black,
    chapterstyle=classic,
    coverstyle=classic,
    aiacknowledgement=true,
    listprefix=false
]{IPLeiriaThesis} % Refer to the Wiki for a list of available options.

%%% Document Version %%%
\DocumentVersion{1.0.0} % Required only if the 'docstage' is set to 'working'.

%%% Document Metadata %%%
\input{Metadata/Metadata}

%%% Additional Packages %%%
\usepackage{float} % For [H] float placement option

%%% Loading of Glossary and Acronyms %%%
\makeglossaries
\loadglsentries{Matter/05-Glossary}
\loadglsentries[\acronymtype]{Matter/06-Acronyms}
\loadglsentries[\symboltype]{Matter/07-Symbols}

\begin{document}

%%% Front Matter %%%
\include{Matter/00-Cover}
% \include{Matter/01-Front-Page}  % REMOVED: Using BITS cover only, no duplicate IPLeiria page

%%% Copyright Statement %%%
\include{Matter/02-Copyright}

%%% Roman Numeration %%%
\pagenumbering{roman}

%%% Acknowledgements %%%
\include{Matter/03-Acknowledgements}

%%% Abstract %%%
\include{Chapters/00-Abstract}

%%% AI Acknowledgement %%%
\include{Matter/04-AI}

%%% Table of Contents, List of Figures and List of Tables %%%
\bookmarktocentry\tableofcontents
\listoffigures
\listoftables

%%% Print: Glossary and Acronyms %%%
\glossarytoc\printnormalglossary
\acronymtoc\printacronymglossary
\symboltoc\printsymbolglossary

%%% Arabic Numeration %%%
\pagenumbering{arabic}

%%% Chapters (**Bits to Atoms Thesis**) %%%
\include{Chapters/00-Preface}
\include{Chapters/01-Introduction}
\include{Chapters/02-Background}
\include{Chapters/03-SystemOverview}
\include{Chapters/04-CMRTA-OIM}
\include{Chapters/05-SNN-MPC}
\include{Chapters/06-Results}
\include{Chapters/07-India}
\include{Chapters/08-Conclusion}
\input{Chapters/Tables/all_tables.tex}

%%% Bibliography %%%
\renewcommand{\refname}{Bibliography}
\printbibliography[title={\refname},heading=bibintoc]

%%% Appendices: Thesis Mathematical Derivations %%%
\appendix
\input{Matter/08-Appendices}
\input{Chapters/Appendices/00-AppendixA}
\input{Chapters/Appendices/00-QUBODerivation}
\input{Chapters/Appendices/01-IsingSigns}
\input{Chapters/Appendices/02-PIPGProof}

%%% Back Page %%%
\input{Matter/10-Back-Page}

\end{document}