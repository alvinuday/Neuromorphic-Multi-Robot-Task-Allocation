...
where:
\begin{itemize}
\item $K_{ii}$ is the \textbf{injection locking strength} (acts as a bias, pulling the oscillator toward a preferred phase).
\item $K_{ij}$ is the \textbf{coupling strength} between oscillators $i$ and $j$.
\item $\xi_i(t)$ is noise (helps escape local minima).
\end{itemize}

The connection to Ising: when two oscillators are \textbf{anti-ferromagnetically coupled} (negative $K_{ij}$), they prefer to oscillate in antiphase ($\theta_i \approx \theta_j + \pi$). When \textbf{ferromagnetically coupled} (positive $K_{ij}$), they prefer in-phase ($\theta_i \approx \theta_j$).
...
\subsection{Strategy 1: Coalition Bounding (CB)}
...
\subsection{Strategy 2: Spatial Proximity Pruning (SP)}
...
\subsection{Strategy 3: Graph Decomposition}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{Figures/fig_4_7.png}
\caption{
\textbf{Coalition Size vs. Hardware Nodes: Scalability Frontier.}
Shows how the number of conflict graph nodes scales with the number of robots for different maximum coalition sizes $k = 1, 2, 3, 4$. Horizontal dashed lines indicate hardware feasibility regions: 100-node OIM (red), 2000-node OIM (orange), 10k-node FeFET (green). The feasibility window shows realistic problem sizes that fit current hardware.
}
\label{fig:coalition-scalability-frontier}
\end{figure}
...
\textbf{Timing breakdown} (for a 100-node conflict graph on current OIM hardware):
\begin{itemize}
\item Pre-processing: 5--10 ms (Python)
\item OIM solve: 15--50 ms (hardware, depends on noise, coupling strength)
\item Post-processing: 2--5 ms (Python greedy repair)
...
\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{Figures/fig_4_8.png}
\caption{
\textbf{OIM Solver Performance: Real-Time Feasibility.}
Mean OIM solve time (blue line with error bars) vs. conflict graph size $|V|$. Orange and red dashed lines indicate real-time deadlines: 20 ms (50 Hz) and 15 ms (66 Hz) respectively. The figure demonstrates that OIM solutions scale gracefully and remain within robotics real-time budgets even for graphs with 300+ nodes, outperforming classical solvers in this regime.
}
\label{fig:oim-solve-times}
\end{figure}
...
\begin{enumerate}
\item \textbf{Local minima on dense graphs:} If the conflict graph is very dense (highly constrained), OIM may converge to a suboptimal independent set. Mitigation: multi-restart OIM (run 10 times, take best), or use noise annealing (gradually reduce noise term).

\item \textbf{Calibration sensitivity:} Real OIM hardware has fabrication mismatch --- oscillator frequencies vary by $\pm 5\%$, couplings vary. This requires periodic calibration. Mitigation: adaptive feedback control, or use statistical ensemble averaging.

\item \textbf{Static problem only:} OIM solves one-shot. If tasks arrive dynamically (common in real warehouses), we must rerun OIM every time. Warm-starting (initializing next solve with previous phases) can help, but is not automatic.

\item \textbf{Coupling latency:} Reprogramming all coupling values takes time (microseconds to milliseconds depending on implementation). For real-time replanning, this overhead must be accounted for.
